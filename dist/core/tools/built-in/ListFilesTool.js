...
export class ListFilesTool {
    ...
    shouldIncludeFile(name, includeHidden) {
    ...
    async listDirectory(dirUri, relativePath, recursive, includeHidden, maxDepth, currentDepth, pattern) {
        const results = [];
        ...
        try {
            ...
            for (const [name, type] of entries) {
                ...
                if (type === vscode.FileType.File) {
                    ...
                    if (recursive) {
                        const subResults = await this.listDirectory(itemUri, itemPath, recursive, includeHidden, maxDepth, currentDepth + 1, pattern);
    ...
    async execute(args, _context) {
        try {
            const { path = '', recursive = false, includeHidden = false, maxDepth = 10, pattern } = args || {};
            ...
            const files = await this.listDirectory(targetUri, '', recursive, includeHidden, maxDepth, 0, pattern);
