...
export async function activate(context) {
    ...
    try {
        ...
        sidebarProvider = new CapCopSidebarProvider(context.extensionUri, providerRegistry, toolRegistry, sessionManager, logger);
        // Register webview provider
        context.subscriptions.push(vscode.window.registerWebviewViewProvider('capcop.sidebar', sidebarProvider, { webviewOptions: { retainContextWhenHidden: true } }));
        // Register commands
...
function registerCommands(context) {
    const commands = [
        ...
        vscode.commands.registerCommand('capcop.insertChange', async (_change) => {
            // TODO: Implement change insertion
            console.log('Insert change command triggered');
        }),
        vscode.commands.registerCommand('capcop.previewEdit', async (_edit) => {
            // TODO: Implement edit preview
            console.log('Preview edit command triggered');
        }),
        vscode.commands.registerCommand('capcop.approveEdit', async (_edit) => {
...
async function showProviderConfiguration() {
    ...
    if (apiKey) {
        ...
        vscode.window.showInformationMessage(localize('configure.provider.success', 'API key configured for {0}', selected.provider.displayName));
    }
}
